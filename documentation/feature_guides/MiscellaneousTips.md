...
3.  Quit the launcher using the `Quit` button
...
You can have multiple campaigns inside the same mod and manually control them from inside the descr_strat file using the following command. Here is an example of how to disable the Marian reforms along with some notes on formatting.
...
6.  Clamp resulting number between `4` and `31`
7.  Multiply the resulting value by the unit scale factor to get the final unit size
